...
async function fetchJson(url) {
    ...
    if (!response.ok) {
        error(`Fehler bei ${url}: ${response.status}`);
...
async function updateAmpel() {
    ...
    try {
        const healthCareCheckup = await fetch('/api/inventory/healthCareCheckup');
        const healthCareCheckupText = await healthCareCheckup.text();
        ...
        if (count >= 4 ||
            expiringProductsResponse.length >= 2 ||
            expiringProductsResponse.length === allProducts.length) {
        ...
        if (healthCareCheckupText === "[Inventar] Unterversorgt! Bitte nachkaufen!") {
            showAmpel("rot-aktiv", true);
            healthCheck = true;
        }
...
/* TOGGLE EDIT */
async function toggleEdit(button, productId, sollIstBoolean) {
    ...
    if (button.innerText === "Ändern") {
        ...
        try {
            if (sollIstBoolean) {
                debug("SollIstBoolean: " + sollIstBoolean);
                if (sollValue !== undefined || istValue !== undefined) {
                    ...
                    debug("responseInventory: " + responseInventory.inventoryId);
                    ...
                    const responseUpdateInventory = await fetch(`api/updateInventory`, {
                        method: 'PUT',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify(inventoryRequest)
                    });

                    if (!responseUpdateInventory.ok) {
                        pushAlertWithOffset(false, "Inventory konnte nicht aktualisiert werden!", 5000, 0);
                        return;
                    }

                    pushAlertWithOffset(true, await responseUpdateInventory.text(), 5000, 0);
                }
            } else {
            ...
            <td>
                <div class="d-flex flex-wrap gap-2">
                    <button type="button"
                            ...
                            onclick="toggleEdit(this, ${product?.id}, false)">
                        Ändern
                    </button>
...
async function performHealthCheck() {
    ...
    try {
        response = await fetch('/api/inventory/healthCareCheckup');
        ...
        error("Health-Check fehlgeschlagen:", err);
    }
    return responseText;
}
...
function renderSollIstRow(p, index, pageData) {
    return `
        <tr data-product-id="${p?.id ?? ""}"
            ...
            <td>
                <button class="btn btn-info btn-sm w-100 mb-2" onclick="toggleEdit(this, ${p.id}, true)">Ändern</button>
...
async function initAddProduct() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        debug("Produkt wird hinzugefügt...");
        ...
        if (!isNameValid || !isUnitValid || !isLagerortValid || !isKategorieValid || !isDateValid || !isSoll || !isIst) {
            ...
            pushAlertWithOffset(false, "Felder dürfen nicht leer sein!", 5000, 0);
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const planStatus = dashboardSection ? dashboardSection.getAttribute('data-plan-status') : 'inactive';
